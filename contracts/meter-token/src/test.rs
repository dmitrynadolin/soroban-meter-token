...
fn test_metadata() {
    ...
    let token = TokenClient::new(&e, &token_contract);

    assert_eq!(token.decimals(), 7);
    assert_eq!(token.name(), String::from_val(&e, &"name"));
    assert_eq!(token.symbol(), String::from_val(&e, &"symbol"));
}
...
fn test_consume() {
    ...
    token.set_supplier(&user, &supplier);

    token.consume(&user, &2000);

    assert_eq!(token.balance(&user), -1000);

    assert_eq!(token.balance(&supplier), 2000);

    assert_eq!(token.get_supplier(&user), Some(supplier.clone()));
}